<script>
    ...
    import {allowedEmailDomains, createInstitutionEduID, institutionalEmailDomains} from "../api";
    ...
    const handleNext = () => {
        if (allowedNext($user.email, $user.givenName, $user.familyName, agreedWithTerms)) {
            showSpinner = true;
            createInstitutionEduID($user.email, $user.givenName, $user.familyName, hash)
                .then(res => {
                    const url = `//${id}?name=${encodeURIComponent(serviceName)}&modus=cr`;
